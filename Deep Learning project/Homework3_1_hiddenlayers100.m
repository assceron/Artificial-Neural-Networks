...
[hiddenThresholds, hiddenWeights, outputThresholds, outputWeights, C_T_net3, C_V_net3,epochForMinimumValid_net3, minimumValid_net3] = train(tTrain, xTrain, xValid, tValid, xTest, tTest, learning_rate, inputsize, outputsize, batchsize, numberOfEpochs, hiddenNeurons);
%%
...
xValid = shift(xValid, meanToShift);
 ...
 C_test_final_net3 = 0.5*mean(C_test);
 ...
 C_train_final_net3 = 0.5*mean(C_train);
 ...
 C_valid_final_net3 = 0.5*mean(C_valid);
%%
